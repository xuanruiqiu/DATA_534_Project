...
### Comparing Any Two Assets

The `plot_asset_comparison()` function creates a normalized price comparison chart for any two assets:

```{r plot-asset-comparison}
# Compare Gold (XAU/USD) vs Bitcoin (BTC/USD) - default
plot_asset_comparison(
  start_date = Sys.Date() - 30,
  end_date = Sys.Date()
)

# Compare two gold ETFs
plot_asset_comparison(
  symbol1 = "GLD",
  symbol2 = "IAU",
  start_date = "2024-01-01",
  end_date = "2024-06-30",
  title = "GLD vs IAU Performance"
)
```

```{r plot-asset-comparison-demo, eval=TRUE, echo=TRUE}
# Create sample comparison data for two assets
set.seed(456)
dates <- seq(Sys.Date() - 29, Sys.Date(), by = "day")
...
p6 <- ggplot(asset_comparison_demo, aes(x = date, y = normalized, color = symbol)) +
  ...
  labs(
    title = "XAU/USD vs BTC/USD - Normalized Price Comparison",
    x = "Date",
